...
static constexpr bool getPublicKeyFromIdentity(const unsigned char *identity, unsigned char *publicKey)
{
	for (int i = 0; i < 32; i++)
	{
		if (identity[i << 1] < 'A' || identity[i << 1] > 'P' || identity[(i << 1) + 1] < 'A' || identity[(i << 1) + 1] > 'P')
		{
			return false;
		}
...
{ // Secret agreement computation for key exchange using a compressed, 32-byte public key
  ...
  // Output: 32-byte sharedKey
	...
	if (A->x[0][0] == 0 && A->x[0][1] == 0 && A->x[1][0] == 0 && A->x[1][1] == 0 && A->y[0][0] == 1 && A->y[0][1] == 0 && A->y[1][0] == 0 && A->y[1][1] == 0) // Is output = neutral point (0,1)?
	{
		return false;
	}
...
static void signMessage(const unsigned char *subseed, const unsigned char *publicKey, const unsigned char *message, const uint64_t messageSize, unsigned char *signature)
{ // SchnorrQ signature generation
  // It produces the signature signature of a message message of size messageSize in bytes
  // Inputs: 32-byte subseed, 32-byte publicKey, and message of size messageSize in bytes
  // Output: 64-byte signature
	point_t R;
	unsigned char k[64], r[64], h[64];
	alignas(32) unsigned char *temp{new unsigned char[messageSize + 64]};
	...
	KangarooTwelve(temp + 32, messageSize + 32, r, 64);
